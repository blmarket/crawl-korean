                                        ...
                                        int reduction = 1)
{
    ...
    {
        /// 두번째 %s는 그냥 생략하고 '그것의' 정도로 번역해도 될 듯 합니다.
    ...
    if (you.hp < you.hp_max)
    {
        ...
        if (heal > 0 && !you.duration[DUR_DEATHS_DOOR])
        {
            ...
            /// 좋아지거나, 아주 좋아지거나.
            mprf(gettext("You feel %sbetter."), 
                (you.hp == you.hp_max) ? gettext("much ") : "");
        }
    }
...
bool melee_attack::player_attack()
{
    ...
    if (ev_margin >= 0)
    {
        ...
        {
            no_damage_message =
                /// 첫번째 %s에는 공격한다는 의미의 동사가 번역되어 나오고,
                /// 이를테면 때렸다, 베었다, 등등.
                /// 두번째 %s에는 맞는 대상이 나옵니다.
...
bool melee_attack::player_aux_test_hit()
{
    ...
    {
        /// 세번째 %s는 생략하고 '그것' 정도로 번역해도 무방함.
        mprf(gettext("Your %s passes through %s as %s momentarily phases out."),
            aux_attack.c_str(),
            defender->name(DESC_NOCAP_THE).c_str(),
...
bool melee_attack::player_aux_apply(unarmed_attack_type atk)
{
    ...
    if (damage_done > 0)
    {
        ...
        {
            mprf(gettext("%s is splashed with acid."),
    ...
    {
        /// 첫번째 %s에는 추가공격명(kick, headbutt, punch 등등)
        /// 두번째에는 맞는 사람
...
std::string melee_attack::player_why_missed(const char *target_name)
{
    ...
    if (to_hit < ev && to_hit + combined_penalty >= ev)
    {
        ...
        if (armour_miss && !shield_miss)
            return make_stringf(gettext("Your %s prevents you from hitting %s."),
                armour_name.c_str(), target_name);
        else if (shield_miss && !armour_miss)
            return make_stringf(gettext("Your shield prevents you from hitting %s."),
                target_name);
        else
            return make_stringf(gettext(
                "Your shield and %s prevent you from hitting %s."),
                armour_name.c_str(), target_name);
    }
...
void melee_attack::player_warn_miss()
{
    ...
    msg::stream << player_why_missed(defender->name(DESC_NOCAP_THE).c_str()) << std::endl;
}
...
int melee_attack::player_hits_monster()
{
    ...
    if (to_hit >= phaseless_evasion && defender_visible)
        /// 두번째 %s는 생략하고 '그것' 정도로 번역해도 무방함.
        msg::stream << make_stringf(gettext(
            "Your attack passes through %s as %s momentarily phases out."),
            defender->name(DESC_NOCAP_THE).c_str(),
            defender->pronoun(PRONOUN_NOCAP).c_str()) << std::endl;
...
int melee_attack::player_weapon_type_modify(int damage)
{
    ...
    case DAM_PIERCE:
        ...
        else
        {
            ...
            {
                const char* pierce_desc[][2] = {{V_("spit"),   N_("like a pig")},
                                                {V_("skewer"), N_("like a kebab")}};
                const int choice = random2(ARRAYSZ(pierce_desc));
                attack_verb = pierce_desc[choice][0];
                verb_degree = gettext(pierce_desc[choice][1]);
            }
        }
        break;
    ...
    case DAM_SLICE:
        ...
        {
            const char* slice_desc[][2] = {{V_("open"),  N_("like a pillowcase")},
                                           {V_("slice"), N_("like a ripe choko")},
                                           {V_("cut"),   N_("into ribbons")}};
            const int choice = random2(ARRAYSZ(slice_desc));
            attack_verb = slice_desc[choice][0];
            verb_degree = gettext(slice_desc[choice][1]);
        }
        break;
    ...
    case DAM_BLUDGEON:
        ...
        else
        {
            const char* bash_desc[][2] = {{V_("crush"),  N_("like a grape")},
                                          {V_("beat"),   N_("like a drum")},
                                          {V_("hammer"), N_("like a gong")},
                                          {V_("pound"),  N_("like an anvil")}};
            const int choice = random2(ARRAYSZ(bash_desc));
            attack_verb = bash_desc[choice][0];
            verb_degree = gettext(bash_desc[choice][1]);
        }
        break;
                                                ...
                                                const char *verb)
{
    ...
    if (needs_message && special_damage > 0 && verb)
    {
        special_damage_message = make_stringf(
            ...
            translate_verb(attacker, verb).c_str(),
...
void melee_attack::rot_defender(int amount, int immediate)
{
    if (defender->rot(attacker, amount, immediate, true))
    {
        ...
        {
            if(amount > 0)
                special_damage_message = make_stringf(gettext("%s rots!"),
                    def_name(DESC_CAP_THE).c_str());
            else
                special_damage_message = make_stringf(gettext("%s looks less resilient!"),
                    def_name(DESC_CAP_THE).c_str());
        }
    }
}
    ...
    if (one_chance_in(3))
    {
        if (defender_visible)
        {
            special_damage_message =
                make_stringf(
                    /// 1. 수비자.
                    gettext("Space warps horribly around %s!"),
                    def_name(DESC_NOCAP_THE).c_str());
        }
...
void melee_attack::antimagic_affects_defender()
{
    if (defender->atype() == ACT_PLAYER)
    {
        ...
        if (!mp_loss)
            return;
        /// 아 맨날 이런 문장만 나오면 얼마나 좋아.
        mpr(gettext("You feel your power leaking away."), MSGCH_WARN);
        dec_mp(mp_loss);
        obvious_effect = true;
    }
             ...
             && !mons_class_flag(defender->type, M_FAKE_SPELLS))
    {
        ...
        /// 1. 수비자의 이름의 소유형
        special_damage_message = make_stringf(gettext("%s magic leaks into the air."), 
                    apostrophise(defender->name(DESC_CAP_THE)).c_str());
        obvious_effect = true;
    }
}
...
void melee_attack::chaos_affects_defender()
{
    ...
    if (attacker == defender)
    {
        ...
        {
            if (defender->atype() == ACT_PLAYER)
                mpr(gettext("You give off a flash of multicoloured light!"));
            else if (you.can_see(defender))
            {
                simple_monster_message(defender->as_monster(),
                                       /// 수비자(몬스터)가 그랬음.
                                       gettext(" gives off a flash of "
                                       "multicoloured light!"));
            }
            else
                mpr(gettext("There is a flash of multicoloured light!"));
        }
    }
    ...
    case CHAOS_CLONE:
    {
        ...
        if (clone_idx != NON_MONSTER)
        {
            ...
            {
                special_damage_message =
                    make_stringf(gettext("%s is duplicated!"),
                                 def_name(DESC_NOCAP_THE).c_str());
            }
...
void melee_attack::chaos_affects_attacker()
{
    ...
    {
        /// 1. 무기 이름.
        mprf(gettext("Smoke pours forth from %s!"), wep_name(DESC_NOCAP_YOUR).c_str());
        ...
        && one_chance_in(200))
    {
        ...
        if (!you.can_see(attacker))
        {
            ...
            if (!noise.empty())
                msg = make_stringf(gettext("You hear %s"), noise.c_str());
        }
        else
        {
...
bool melee_attack::apply_damage_brand()
{
        ...
        else if (one_chance_in(3))
        {
            special_damage_message =
                defender->atype() == ACT_PLAYER?
                   gettext("You are electrocuted!")
                :  gettext("There is a sudden explosion of sparks!");
    ...
    case SPWPN_VAMPIRICISM:
    {
        ...
        else if (attacker_visible)
        {
            ...
            {
                /// 1. 공격자의 이름.
                mprf(gettext("%s draws strength from your injuries!"),
                     attacker->name(DESC_CAP_THE).c_str());
            }
            else
            {
                /// 1. 공격자의 이름.
                mprf(gettext("%s is healed."),
                     attacker->name(DESC_CAP_THE).c_str());
            }
        }
        ...
        && (dam >= 4 || wpn_brand == SPWPN_VORPAL || coinflip()))
    {
        ...
        else
        {
            ...
            if (defender->holiness() == MH_NATURAL)
            {
                ...
                {
                    if (defender_visible)
                        mpr(gettext("The flame cauterises the wound!"));
                    return (true);
                }
                else if (defender->as_monster()->number < limit - 1)
                {
                    simple_monster_message(defender->as_monster(),
                                           gettext(" grows two more!"));
...
void melee_attack::player_apply_staff_damage()
{
    ...
    case STAFF_AIR:
        ...
        {
            special_damage_message =
                make_stringf(gettext("%s is electrocuted!"),
                             defender->name(DESC_CAP_THE).c_str());
            special_damage_flavour = BEAM_ELECTRICITY;
        }
        ...
        && attacker != defender)
    {
        ...
        if (attacker->as_monster()->friendly())
        {
            if (you.pet_target == MHITYOU || you.pet_target == MHITNOT)
            {
                ...
                {
                    mpr(gettext("Zin prevents your ally from violating sanctuary "
...
bool melee_attack::mons_attack_warded_off()
{
        ...
        && attacker->as_monster()->check_res_magic(warding) <= 0)
    {
        if (needs_message)
        {
            mprf(gettext("%s tries to attack %s, but flinches away."),
                 atk_name(DESC_CAP_THE).c_str(),
                 mons_defender_name().c_str());
        }
        return (true);
...
std::string melee_attack::mons_attack_desc(const mon_attack_def &attk)
{
    ...
    if (dist > 2)
    {
        ...
        ret = gettext(" from afar");
    }

    if (weapon && attacker->type != MONS_DANCING_WEAPON)
        ret += " with " + weapon->name(true, DESC_NOCAP_A);
...
void melee_attack::mons_announce_hit(const mon_attack_def &attk)
{
    if (needs_message)
    {
        /// 1. 공격자, 2. 공격 동사, 3. 대상, 4. 디버깅 메시지, 5. 부가 설명, 6. 느낌표.
        /// 대충 1. 5. 2. 3. 4. 6. 정도로 배열하면 될 듯 합니다.
        mprf(gettext("%s %s %s%s%s%s"),
             atk_name(DESC_CAP_THE).c_str(),
             translate_verb(attacker, mons_attack_verb(attk).c_str()).c_str(),
             mons_defender_name().c_str(),
             debug_damage_number().c_str(),
            ...
            && one_chance_in(attk.flavour == AF_POISON ? 4 : 3)))
    {
        if (needs_message)
        {
                ...
                && (attk.type == AT_BITE || attk.type == AT_STING))
            {
                ...
                {
                    /// 1. 공격자 2. 공격 동사
                    mprf(gettext("%s %s was poisonous!"),
...
void melee_attack::mons_do_napalm()
{
    ...
    if (one_chance_in(20) || (damage_done > 2 && one_chance_in(3)))
    {
        if (needs_message)
        {
            /// 1. 대상 2. is/are 3. 마침표나 느낌표
            mprf(gettext("%s %s covered in liquid flames%s"),
                 def_name(DESC_CAP_THE).c_str(),
...
void melee_attack::splash_monster_with_acid(int strength)
{
    ...
    if (defender_visible)
        mprf(gettext("%s is splashed with acid."), defender->name(DESC_CAP_THE).c_str());
...
void melee_attack::mons_apply_attack_flavour(const mon_attack_def &attk)
{
    ...
    case AF_VAMPIRIC:
        ...
        if (defender->stat_hp() < defender->stat_maxhp())
        {
            ...
            {
                /// 공격자, draws, 대상
                mprf(gettext("%s draws strength from %s injuries!"),
                     atk_name(DESC_CAP_THE).c_str(),
    ...
    case AF_CONFUSE:
        if (attk.type == AT_SPORE)
        {
            ...
            {
                /// 대상, are/is
                mprf(gettext("%s %s engulfed in a cloud of spores!"),
                     defender->name(DESC_CAP_THE).c_str(),
                     defender->conj_verb("are").c_str());
            }
        }
    ...
    case AF_STEAL_FOOD:
    {
        ...
        if (needs_message)
        {
            if (stolen)
            {
                mprf(gettext("%s devours some of your food!"),
                     atk_name(DESC_CAP_THE).c_str());
            }
            else if (ground)
            {
                mprf(gettext("%s devours some of the food beneath you!"),
                     atk_name(DESC_CAP_THE).c_str());
            }
        }
        break;
...
void melee_attack::mons_do_passive_freeze()
{
        ...
        && grid_distance(you.pos(), attacker->as_monster()->pos()) == 1)
    {
        ...
        simple_monster_message(mon, gettext(" is very cold."));
...
void melee_attack::mons_do_eyeball_confusion()
{
        ...
        && x_chance_in_y(player_mutation_level(MUT_EYEBALLS), 20))
    {
        ...
        {
            mprf(gettext("The eyeballs on your body gaze at %s."),
...
void melee_attack::mons_do_spines()
{
    ...
    if (you.mutation[MUT_SPINY] && attacker->alive() && one_chance_in(evp + 1))
    {
        ...
        {
            simple_monster_message(attacker->as_monster(),
                                   gettext(" is struck by your spines."));
        }
...
bool melee_attack::do_trample()
{
    do
    {
        ...
        if (needs_message)
        {
            ...
            mprf(gettext("%s %s backwards!"),
                 def_name(DESC_CAP_THE).c_str(),
                 translate_verb(defender, N_("stumble")).c_str());
        }
    ...
    if (needs_message)
    {
        ...
        mprf("%s %s %s ground!",
             def_name(DESC_CAP_THE).c_str(),
             translate_verb(defender, N_("hold")).c_str(),
            ...
            else if (test_melee_hit(to_hit, defender_evasion, r) >= 0)
            {
                if (needs_message)
                {
                    ...
                    mprf(gettext("Helpless, %s %s to dodge %s attack."),
                         mons_defender_name().c_str(),
                         translate_verb(defender, N_("fail")).c_str(),
                         atk_name(DESC_NOCAP_ITS).c_str());
                }
                this_round_hit = true;
            }
            else if (test_melee_hit(to_hit, defender_evasion_nophase, r) >= 0)
            {
                if (needs_message)
                {
                    ...
                    mprf(gettext("%s momentarily %s out as %s "
                         ...
                         defender->name(DESC_CAP_THE).c_str(),
                         translate_verb(defender, N_("phase")).c_str(),
                         atk_name(DESC_NOCAP_ITS).c_str(),
...
static void stab_message(actor *defender, int stab_bonus)
{
    ...
    case 6:     // big melee, monster surrounded/not paying attention
        if (coinflip())
        {
            mprf( you.species == SP_FELID 
                ? gettext("You pounce on %s from a blind spot!")
                : gettext("You strike %s from a blind spot!")
                  ,defender->name(DESC_NOCAP_THE).c_str());
        }
        else
        {
            mprf(gettext("You catch %s momentarily off-guard."),
                  defender->name(DESC_NOCAP_THE).c_str());
        }
        break;
    case 4:     // confused/fleeing
        if (!one_chance_in(3))
        {
            mprf(gettext("You catch %s completely off-guard!"),
                  defender->name(DESC_NOCAP_THE).c_str());
        }
        else
        {
            mprf( you.species == SP_FELID 
                ? gettext("You pounce on %s from behind!")
                : gettext("You strike %s from a behind!")
                  ,defender->name(DESC_NOCAP_THE).c_str());
        }
        break;
    case 2:
    case 1:
        ...
        {
            mprf(gettext("You pounce on the unaware %s!"),
                 defender->name(DESC_PLAIN).c_str());
            break;
        }
