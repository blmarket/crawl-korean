...
void unlink_item(int dest)
{
    ...
    if (mons != NULL)
    {
        ...
        mprf(MSGCH_ERROR, _("Item %s claims to be held by monster %s, but "
             ...
             mitm[dest].name(true, DESC_PLAIN, false, true).c_str(),
...
static std::string _menu_burden_invstatus(const Menu *menu, bool is_pickup = false)
{
    ...
    std::string newstate = new_burd > carrying_capacity(BS_ENCUMBERED) ?
                           ...
                           new_burd > carrying_capacity(BS_UNENCUMBERED) ?
                               _("burdened)") : _("unencumbered)");

    std::string burden = _("(Burden: ");
    ...
    if (item.orig_monnum)
    {
        if (item.orig_monnum < 0)
        {
            ...
            case AQ_CARD_GENIE:
                ...
                break;
            case AQ_WIZMODE:
                desc += _("Your wizardly powers created ")+ _article_it(item)+ P_("wizaq"," ");
                break;
            default:
                if (iorig > GOD_NO_GOD && iorig < NUM_GODS)
                {
			...
			gettext(_article_it(item).c_str()));
                ...
                {
                    // Bug really.
                    desc += _("You stumbled upon ") + _article_it(item) + P_("item"," ");
                }
                break;
            }
        }
...
static void _rune_from_specs(const char* _specs, item_def &item)
{
    ...
    if (strlen(specs) > 4)
    {
        for (int i = 0; i < NUM_RUNE_TYPES; ++i)
        {
            ...
            if (lowercase_string(item.name(false, DESC_PLAIN)).find(specs) != std::string::npos)
                return;
        }
    }
