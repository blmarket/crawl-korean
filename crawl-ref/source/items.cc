...
void item_check(bool verbose)
{
    ...
    {
        if (verbose)
            strm << _("There are no items here.") << endl;
        return;
    }
    ...
    if (static_cast<int>(items.size()) >= Options.item_stack_summary_minimum)
    {
        ...
        string out_string = _("Items here: ");
        int cur_state = -1;
    ...
    {
        if (!done_init_line)
            mprnojoin(gettext("Things that are here:"), MSGCH_FLOOR_ITEMS);
        ...
        strm << _("There are many items here.") << endl;
...
#ifdef TOUCH_UI
    string prompt = _("Pick up what? (<Enter> or tap header to pick up)");
#else
    string prompt = _("Pick up what? (_ for help)");
#endif
...
void pickup(bool partial_quantity)
{
    ...
    {
        int next;
        mpr(_("There are several objects here."));
        string pickup_warning;
        while (o != NON_ITEM)
        {
            ...
            if (keyin != 'a')
            {
                string prompt = _("Pick up %s? ((y)es/(n)o/(a)ll/(m)enu/*?g,/q)");
                        ...
                        bool ignore_burden)
{
    ...
    if (it.base_type == OBJ_ORBS && crawl_state.game_is_zotdef())
    {
        if (runes_in_pack() < 15)
        {
            mpr(_("You must possess at least fifteen runes to touch the sacred Orb which you defend."));
            return 1;
        }
    }
...
bool drop_item(int item_dropped, int quant_drop)
{
    ...
    {
        mprf(_("%s is stuck to you!"), you.inv[item_dropped].name(true, DESC_THE).c_str());
        return false;
    }
    ...
    {
        mpr(_("Too many items on this level, not dropping the item."));
        return false;
    }
...
#ifdef TOUCH_UI
    tmp_items = prompt_invent_items(_("Drop what? (<Enter> or tap header to drop)"),
                                    MT_DROP,
#else
    tmp_items = prompt_invent_items(_("Drop what? (_ for help)"), MT_DROP,
#endif
...
static void _rune_from_specs(const char* _specs, item_def &item)
{
    ...
    if (strlen(specs) > 4)
    {
        for (int i = 0; i < NUM_RUNE_TYPES; ++i)
        {
            ...
            if (lowercase_string(item.name(false, DESC_PLAIN)).find(specs) != string::npos)
                return;
        }
    }
...
static void _deck_from_specs(const char* _specs, item_def &item)
{
    ...
    if (!type_str.empty())
    {
        for (int i = 0; types[i] != NUM_MISCELLANY; ++i)
        {
            ...
            string name = item.name(false, DESC_PLAIN).substr(6);
            item.props.clear();
                      ...
                      object_class_type class_wanted, bool create_for_real)
{
    ...
    if (!item->sub_type)
    {
        ...
        {
            item->sub_type = i;
            size_t pos = lowercase_string(item->name(false, DESC_PLAIN)).find(specs);
            if (pos != string::npos)
            {
                ...
                {
                    if (create_for_real)
                        mpr(item->name(true, DESC_PLAIN).c_str());
                    type_wanted = i;
                    best_index = pos;
                }
            }
    ...
    case OBJ_ARMOUR:
    {
        ...
        if (buf[0] != '\0')
        {
            ...
            {
                item->special = i;
                size_t pos = lowercase_string(item->name(false, DESC_PLAIN)).find(buf_lwr);
                if (pos != string::npos)
                {
                    // earliest match is the winner
                    if (pos < best_index)
...
void corrode_item(item_def &item, actor *holder)
{
    ...
    else if (holder && you.see_cell(holder->pos()))
    {
        ...
        {
            mprf(_("The acid corrodes %s!"),
                 holder->name(DESC_THE).c_str());
        }
        else
        {
            mprf(_("The acid corrodes %s %s!"),
