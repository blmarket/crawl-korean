...
static string _get_version_changes(void)
{
    ...
    if (start)
    {
        ...
        result += "\n\n";
        result += gettext("For earlier changes, see changelog.txt "
...
static const char *targetting_help_2 =
    N_("<h>Targetting (zap wands, cast spells, etc.):\n"
    ...
    for (int i = 0; i < NUM_RODS; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
            if (which_spell_in_book(book, j) == type)
                rods.push_back(item.name(false, DESC_BASENAME));
    }
...
static void _add_formatted_keyhelp(column_composer &cols)
{
    ...
    _add_command(cols, 0, CMD_WAIT, _("wait a turn (also <w>.</w>, <w>Del</w>)"), 2);
    _add_command(cols, 0, CMD_REST, _("rest and long wait; stops when"), 2);
    cols.add_formatted(
            0,
    ...
    _add_command(cols, 0, CMD_EXPLORE, _("auto-explore"));
    _add_command(cols, 0, CMD_INTERLEVEL_TRAVEL, _("interlevel travel"));
    _add_command(cols, 0, CMD_SEARCH_STASHES, _("Find items"));
    _add_command(cols, 0, CMD_FIX_WAYPOINT, _("set Waypoint"));
    ...
    {
        string interact = (you.species == SP_VAMPIRE ? _("Drain corpses")
                                                     : _("Eat food"));
        interact += _(" (tries floor first)\n");
    ...
    {
        string interact = (you.species == SP_VAMPIRE ? _("Drain corpses on")
                                                     : _("Eat food from"));
    ...
    string text =
            _("Many commands have context sensitive "
...
int list_wizard_commands(bool do_redraw_screen)
{
    ...
    cols.add_formatted(1,
                       _("<yellow>Other player related effects</yellow>\n"
                       "<w>c</w>      card effect\n"
#ifdef DEBUG_BONES
