                     ...
                     int foe, bool fail)
{
    ...
    if (!mon)
    {
        mpr(gettext("Failed to spawn projectile."), MSGCH_ERROR);
        return SPRET_ABORT;
    }
...
move_again:
    ...
    if (cell_is_solid(pos) || victim)
    {
        ...
        {
            if (you.see_cell(pos))
                mprf(gettext("%s hits %s"), mon.name(DESC_THE, true).c_str(),
        ...
        if (victim && _iood_shielded(mon, *victim))
        {
            ...
            if (!shield_reflects(*shield))
            {
                ...
                {
                    mprf(gettext("You block %s."), mon.name(DESC_THE, true).c_str());
                }
                else
                {
                    simple_monster_message(mons, (pgettext("iood"," blocks ")
                        + mon.name(DESC_THE, true) + pgettext("iood",".")).c_str());
                }
        ...
        if (victim == &you)
            mprf(gettext("%s hits you!"), mon.name(DESC_THE, true).c_str());
