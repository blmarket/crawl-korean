...
void bolt::fire_wall_effect()
{
    ...
    nuke_wall(pos());
    if (you.see_cell(pos()))
        emit_message(MSGCH_PLAIN, _("The tree burns like a torch!"));
    else if (you.can_smell())
        emit_message(MSGCH_PLAIN, _("You smell burning wood."));
...
static bool _nuke_wall_msg(dungeon_feature_type feat, const coord_def& p)
{
    ...
    case DNGN_GRATE:
        ...
        {
            if (see)
                msg = gettext("The grate screeches as it bends and collapses.");
            else
...
bool bolt::hit_wall()
{
        ...
        && !feat_is_solid(grd(target)))
    {
        ...
        string prompt = _("Your line of fire to ");
        ...
        if (actor *act = actor_at(pos()))
        {
            ...
            else if (!is_tracer && you.can_see(act))
            {
                mprf(_("The %s protects %s from harm."),
                     _(raw_feature_description(act->pos()).c_str()), 
                          ...
                          bool doFlavouredEffects)
{
    ...
    case BEAM_STEAM:
        ...
        {
            if (mons->is_icy())
                simple_monster_message(mons, _(" melts!"));
            else if (mons_species(mons->type) == MONS_BUSH
                     && mons->res_fire() < 0)
            {
                simple_monster_message(mons, _(" is on fire!"));
            }
    ...
    case BEAM_WATER:
        ...
        {
            if (!hurted)
                simple_monster_message(mons, _(" shrugs off the wave."));
            else if (hurted > original)
                simple_monster_message(mons, _(" is doused terribly!"));
        }
        break;
...
void bolt::affect_endpoint()
{
    ...
    if (origin_spell == SPELL_PRIMAL_WAVE) // &&coinflip()
    {
        if (you.see_cell(pos()))
        {
            ...
            noisy(25, pos(), _("You hear a splash."));
...
void bolt::affect_player()
{
    ...
    if (item && item->base_type == OBJ_MISSILES)
    {
        ...
        if (you.mutation[MUT_JELLY_MISSILE]
            ...
            && coinflip())
        {
            mprf(_("Your attached jelly eats %s!"), item->name(true, DESC_PLAIN).c_str());
        ...
        if (name == "splash of liquid fire")
            for (adjacent_iterator ai(source); ai; ++ai)
            {
                ...
                if (actor *victim = actor_at(*ai))
                {
                    if (you.see_cell(*ai))
                    {
                        mprf(_("The sticky flame splashes onto %s!"),
...
void bolt::affect_monster(monster* mon)
{
    ...
    if (fedhas_shoot_through(*this, mon) && !is_tracer)
    {
        ...
        {
            simple_god_message(
                make_stringf(_(" protects %s plant from harm."),
    ...
    if (!engulfs && !_test_beam_hit(beam_hit, rand_ev, is_beam, defl, r))
    {
        ...
        if (mon->observable())
        {
            ...
            {
                mprf(defl == 2 ? _("%s deflects the %s!") : _("%s repels the %s!"),
                     mon->name(DESC_THE).c_str(), _(name.c_str()));
            }
            ...
            {
                mprf(_("The %s misses %s."), _(name.c_str()),
                     mon->name(DESC_THE).c_str());
            }
        }
        return;
...
mon_resist_type bolt::apply_enchantment_to_monster(monster* mon)
{
    ...
    case BEAM_ENSLAVE_SOUL:
    {
        ...
        {
            simple_monster_message(mon, _("'s soul is too badly injured."));
            return MON_OTHER;
        }
        ...
        simple_monster_message(mon, _("'s soul is now ripe for the taking."));
        return MON_AFFECTED;
    }
...
string bolt::get_short_name() const
{
    ...
    if (item != NULL && item->defined())
        return item->name(false, DESC_A, false, false, false, false,
