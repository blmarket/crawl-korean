    ...
    if (m->is_constricted())
    {
        ...
        if (constrictor)
        {
            constrictor_name = constrictor->name(DESC_PLAIN, true) +  
                               (m->held == HELD_MONSTER ? pgettext("moninfo","held by ") 
                                                        : pgettext("moninfo","constricted by ")); 
        }
    }
        ...
        for (i = m->constricting->begin(); i != m->constricting->end(); ++i)
        {
            ...
            if (constrictee)
            {
                constricting_name.push_back(
					            constrictee->name(DESC_PLAIN, true) +
                                (m->constriction_damage() ? pgettext("moninfo","constricting ")
                                                          : pgettext("moninfo","holding ")));                               
            }
        }
    }
    ...
    else
    {
        ...
        case MONS_SPECTRAL_WEAPON:
            if (inv[MSLOT_WEAPON].get())
            {
                ...
                s = type==MONS_SPECTRAL_WEAPON ? _("spectral ") : "";
                s += (item.name(true, DESC_PLAIN, false, false, use_inscrip, false,
                                ignore_flags));
            }
            break;
...
vector<string> monster_info::attributes() const
{
    ...
    if (is(MB_ROUSED))
        v.push_back(pgettext("attributes","inspired to greatness"));
    if (is(MB_HASTED))
    ...
    if (is(MB_BLIND))
        v.push_back(pgettext("attributes","blind"));
    if (is(MB_DUMB))
    ...
    if (is(MB_STONESKIN))
        v.push_back(pgettext("attributes","stone skin"));
    if (is(MB_OZOCUBUS_ARMOUR))
        v.push_back(pgettext("attributes","covered in an icy film"));
    if (is(MB_WRETCHED))
        v.push_back(pgettext("attributes","misshapen and mutated"));
    if (is(MB_WORD_OF_RECALL))
        v.push_back(pgettext("attributes","chanting recall"));
    if (is(MB_INJURY_BOND))
        v.push_back(pgettext("attributes","sheltered from injuries"));
    if (is(MB_WATER_HOLD))
        v.push_back(pgettext("attributes","engulfed in water"));
    if (is(MB_WATER_HOLD_DROWN))
    {
        v.push_back(pgettext("attributes","engulfed in water"));
        v.push_back(pgettext("attributes","unable to breathe"));
    }
    if (is(MB_FLAYED))
        v.push_back(pgettext("attributes","covered in terrible wounds"));
    if (is(MB_RETCHING))
        v.push_back(pgettext("attributes","retching with violent nausea"));
    if (is(MB_WEAK))
        v.push_back(pgettext("attributes","weak"));
    if (is(MB_DIMENSION_ANCHOR))
        v.push_back(pgettext("attributes","unable to translocate"));
...
string monster_info::wounds_description_sentence() const
{
    ...
    if (wounds.empty())
        ...
        return string(pronoun(PRONOUN_SUBJECTIVE)) + _(" is ") + wounds + ".";
}
