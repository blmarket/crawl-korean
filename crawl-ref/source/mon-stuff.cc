...
static bool _ely_protect_ally(monster* mons, killer_type killer)
{
    ...
    snprintf(info, INFO_SIZE, _(" protects %s from harm!"),
                ...
                int killer_index, bool silent, bool wizard, bool fake)
{
    ...
    if (killer == KILL_YOU && you.berserk())
    {
                 ...
                 && one_chance_in(30))
        {
            const int bonus = (2 + random2(4)) / 2;
            you.increase_duration(DUR_BERSERK, bonus);
            mpr(_("Your amulet glows a violent red."));
        }
    }
             ...
             && !mons->pacified() && (!summoned || duration > 0) && !wizard)
    {
        if (you.can_see(mons))
        {
            mprf(MSGCH_WARN, _("Miasma billows from the fallen %s!"),
                 mons->name(DESC_PLAIN).c_str());
        }
                       ...
                       bool force_beh)
{
    ...
    if (targetc == RANDOM_MONSTER)
    {
        do
        {
            ...
            if (relax > 50)
                return simple_monster_message(mons, _(" shudders."));
        }
    ...
    if (could_see)
    {
        ...
        mprf(verb.c_str(), old_name_the.c_str(), _(obj.c_str()));
    }
    else if (can_see)
    {
...
bool swap_places(monster* mons, const coord_def &loc)
{
    ...
    if (monster_at(loc))
    {
        mpr(_("Something prevents you from swapping places."));
        return false;
    }
...
bool swap_check(monster* mons, coord_def &loc, bool quiet)
{
    ...
    if (!swap && !quiet)
    {
        ...
        simple_monster_message(mons, _(" cannot make way for you."));
...
string summoned_poof_msg(const monster* mons, bool plural)
{
    ...
    string msg      = _("disappear%s in a puff of smoke");
    bool   no_chaos = false;
...
static bool _mons_reaped(actor *killer, monster* victim)
{
    ...
    if (you.can_see(victim))
        ...
        mprf(gettext("%s appears out of thin air!"), zombie->name(DESC_THE).c_str());
