                     ...
                     bool check_validity)
{
    ...
    case SPELL_BOLT_OF_INACCURACY:
        beam.name     = M_("narrow beam of energy");
    ...
    case SPELL_PRIMAL_WAVE:
        beam.name     = M_("great wave of water");
    ...
    case SPELL_LIGHTNING_BOLT:
        beam.name     = M_("bolt of lightning");
    ...
    case SPELL_ICE_STORM:
        beam.name           = M_("great blast of cold");
        beam.colour         = BLUE;
    ...
    case SPELL_BOLT_OF_DRAINING:      // negative energy
        beam.name     = M_("bolt of negative energy");
        beam.damage   = dice_def(3, 6 + power / 13);
        beam.colour   = DARKGREY;
        beam.flavour  = BEAM_NEG;
    ...
    case SPELL_ISKENDERUNS_MYSTIC_BLAST: // mystic blast
        beam.colour     = LIGHTMAGENTA;
        beam.name       = M_("orb of energy");
        beam.short_name = M_("energy");
    ...
    case SPELL_QUICKSILVER_BOLT:   // Quicksilver dragon and purple draconian
        ...
        beam.name       = M_("bolt of dispelling energy");
    ...
    case SPELL_METAL_SPLINTERS:
        beam.name       = M_("spray of metal splinters");
    ...
    case SPELL_HOLY_BREATH:
        beam.name     = M_("blast of cleansing flame");
        beam.damage   = dice_def(3, (mons->hit_dice * 2));
    ...
    case SPELL_PETRIFYING_CLOUD:
        beam.name     = M_("blast of calcifying dust");
        beam.colour   = WHITE;
    ...
    case SPELL_HOLY_LIGHT:
        beam.name     = M_("beam of golden light");
        beam.damage   = dice_def(3, 8 + power / 11);
        beam.colour   = ETC_HOLY;
        beam.flavour  = BEAM_HOLY;
        beam.hit      = 17 + power / 25;
                                  ...
                                  int wont_attack)
{
    ...
    if (you.religion == GOD_SHINING_ONE)
    {
        ...
        if (pow < duration)
        {
            simple_god_message(_(" protects your fellow warrior from evil "
                               "magic!"));
            shielded = true;
        }
    }
    else if (you.religion == GOD_TROG)
    {
        ...
        if (pow < duration)
        {
            simple_god_message(_(" shields your ally from puny magic!"));
            shielded = true;
        }
    }
    ...
    {
        pow = 0;
        mpr(_("Zin's power protects your fellow warrior from evil magic!"),
            MSGCH_GOD);
        shielded = true;
    }
    ...
    {
        if (!shielded)
            simple_monster_message(target, _(" shudders."));
        return 1;
    }
...
static int _monster_abjuration(const monster* caster, bool actual)
{
    ...
    if (actual)
        mpr(_("Send 'em back where they came from!"));
...
static bool _mons_vampiric_drain(monster *mons)
{
    ...
    if (!hp_cost)
    {
        simple_monster_message(mons,
                               _(" is infused with unholy energy, but nothing happens."),
                               MSGCH_MONSTER_SPELL);
        return false;
    }
    ...
    {
        simple_monster_message(mons,
                               _(" is infused with unholy energy."),
                               MSGCH_MONSTER_SPELL);
    }
    else
        mpr(_("Unholy energy fills the air."));
...
static int _mons_mesmerise(monster* mons, bool actual)
{
    ...
    {
        simple_monster_message(mons, _(" attempts to bespell you!"));
...
static int _mons_cause_fear(monster* mons, bool actual)
{
    ...
    {
        if (you.can_see(mons))
            simple_monster_message(mons, _(" radiates an aura of fear!"));
        else
            mpr(_("An aura of fear fills the air!"));
    ...
    for (actor_iterator ai(mons->get_los()); ai; ++ai)
    {
        ...
        else
        {
                ...
                && m->add_ench(mon_enchant(ENCH_FEAR, 0, mons)))
            {
                ...
                if (you.can_see(m))
                    simple_monster_message(m, _(" looks frightened!"));
...
static bool _mons_drain_life(monster* mons, bool actual)
{
    if (actual && you.see_cell(mons->pos()))
    {
        ...
        {
            simple_monster_message(mons,
                                   _(" draws from the surrounding life force!"));
        }
        else
            mpr(_("The surrounding life force dissipates!"));
...
static bool _mons_ozocubus_refrigeration(monster* mons, bool actual)
{
    if (actual)
    {
        ...
        {
            simple_monster_message(mons,
                                   _(" drains the heat from the surrounding"
                                   " environment!"));
        }
        else
            mpr(_("The ambient heat is drained!"));
...
static void _mons_create_tentacles(monster* head)
{
    ...
    {
        if (visible_count == 1)
            mpr(_("A tentacle rises from the water!"));
        else if (visible_count > 1)
            mpr(_("Tentacles burst out of the water!"));
    }
    else if (head->type == MONS_TENTACLED_STARSPAWN)
    {
        if (visible_count == 1)
            mpr(_("A tentacle flies out from the starspawn's body!"));
        else if (visible_count > 1)
            mpr(_("Tentacles burst from the starspawn's body!"));
    }
               ...
               bool do_noise, bool special_ability)
{
    ...
    case SPELL_MIRROR_DAMAGE:
        simple_monster_message(mons,
                               make_stringf(_(" offers %s to %s, and fills with unholy energy."),
                                   ...
                                   _(god_name(mons->god).c_str())).c_str(),
                               MSGCH_MONSTER_SPELL);
    ...
    case SPELL_SWIFTNESS:
        ...
        if (mons->type == MONS_ALLIGATOR)
        {
            ...
            simple_monster_message(mons, _(" puts on a burst of speed!"));
        }
        else
            simple_monster_message(mons, _(" seems to move somewhat quicker."));
        return;

    case SPELL_STONESKIN:
    {
        if (you.can_see(mons))
            mprf(_("%s skin hardens."),
    ...
    case SPELL_SILENCE:
        ...
        simple_monster_message(mons, _("'s surroundings become eerily quiet."));
        return;
    ...
    case SPELL_CALL_TIDE:
        if (player_in_branch(BRANCH_SHOALS))
        {
            ...
            if (simple_monster_message(
                    mons,
                    _(" sings a water chant to call the tide!")))
            {
    ...
    case SPELL_INK_CLOUD:
        ...
        simple_monster_message(
            mons,
            _(" squirts a massive cloud of ink into the water!"));
        return;
    ...
    case SPELL_LEDAS_LIQUEFACTION:
        ...
        {
            mprf(_("%s liquefies the ground around %s!"), mons->name(DESC_THE).c_str(),
                 mons->pronoun(PRONOUN_REFLEXIVE).c_str());
    ...
    case SPELL_CANTRIP:
    {
        ...
        else
        {
            ...
            const char* buff_msgs[] = { N_(" glows brightly for a moment."),
                                        N_(" looks stronger."),
                                        N_(" becomes somewhat translucent."),
                                        N_("'s eyes start to glow.") };

            // Messages about the monster influencing you.
            const char* other_msgs[] = {
                N_("You feel troubled."),
                N_("You feel a wave of unholy energy pass over you.")
            };

            if (buff_only || crawl_state.game_is_arena() || x_chance_in_y(2,3))
            {	std::string buff_msgs_temp = RANDOM_ELEMENT(buff_msgs);
                simple_monster_message(mons, _(buff_msgs_temp.c_str()),
                                       channel);
            }
            else if (friendly)
            {
                simple_monster_message(mons, _(" shimmers for a moment."),
                                       channel);
            }
            else // "Enchant" the player.
            {
                mons_cast_noise(mons, pbolt, spell_cast);	std::string other_msgs_temp = RANDOM_ELEMENT(other_msgs);
                mpr(_(other_msgs_temp.c_str()));
            }
        }
        return;
    }
    ...
    case SPELL_AWAKEN_FOREST:
        ...
        forest_message(mons->pos(), _("The forest starts to sway and rumble!"));
        return;
    ...
    case SPELL_DEATHS_DOOR:
        if (!mons->has_ench(ENCH_DEATHS_DOOR))
        {
            ...
            simple_monster_message(mons,
                                   _(" stands defiantly in death's doorway!"));
    ...
    {
        simple_monster_message(mons,
                               _("'s wounds begin to heal before your eyes!"));
        const int dur = BASELINE_DELAY
            * min(5 + roll_dice(2, (mons->hit_dice * 10) / 3 + 1), 100);
        mons->add_ench(mon_enchant(ENCH_REGENERATION, 0, mons, dur));
        return;
    ...
    case SPELL_OZOCUBUS_ARMOUR:
    {
        mprf(_("A film of ice covers %s body!"),
        apostrophise(mons->name(DESC_THE)).c_str());
        const int power = (mons->hit_dice * 15) / 10;
