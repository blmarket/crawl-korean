...
void melee_attack::player_aux_setup(unarmed_attack_type atk)
{
    ...
    case UNAT_HEADBUTT:
        ...
        {
            aux_attack = aux_verb = P_("verb", "peck");
            aux_damage++;
            noise_factor = 75;
        }
        else
...
void melee_attack::chaos_affects_attacker()
{
        ...
        && one_chance_in(200))
    {
        ...
        if (!you.can_see(attacker))
        {
            ...
            if (!noise.empty())
                msg = make_stringf(gettext("You hear %s"),
                                   noise.c_str());
        }
        else
        {
...
void melee_attack::do_minotaur_retaliation()
{
    ...
    if (!defender->is_player())
    {
        ...
        {
            const std::string defname = defender->name(DESC_THE);
            mprf(gettext("%s furiously retaliates!"), defname.c_str());
            if (hurt <= 0)
            {
    ...
    if (5 * you.strength() + 7 * you.dex() > random2(600))
    {
        ...
        mpr(gettext("You furiously retaliate!"));
