...
bool melee_attack::handle_phase_attempted()
{
    ...
    if (attacker->is_player() && defender->is_monster())
    {
                ...
                && weapon->special == UNRAND_DEVASTATOR))
        {
            ...
            {
                string prompt = make_stringf(_("Really attack with "
                    "%s while in water? "),
                    weapon ? weapon->name(true, DESC_YOUR).c_str()
                        : _(M_("your electric unarmed attack")));
                 ...
                 && !crawl_state.game_is_zotdef())
        {
            ...
            mprf(_("The %s protects %s from harm."),
                 _(raw_feature_description(defender->pos()).c_str()),
    ...
    {
        string feat_name = _(raw_feature_description(defender->pos()).c_str());
        ...
        if (attacker->is_player())
        {
            if (you.can_see(defender))
            {
                ...
                mprf(_("You hit the %s."), feat_name.c_str());
        }
        else
        {
...
bool melee_attack::player_aux_apply(unarmed_attack_type atk)
{
    ...
    else // no damage was done
    {
        mprf(gettext("You %s %s%s."),
             pgettext_expr("verb", aux_verb.c_str()),
             defender->name(DESC_THE).c_str(),
...
bool melee_attack::player_monattk_hit_effects()
{
        ...
        && mons_class_is_confusable(defender->type))
    {
        ...
        {
            mprf(gettext("%s is stunned!"), defender->name(DESC_THE).c_str());
        }
    }
...
void melee_attack::rot_defender(int amount, int immediate)
{
    if (defender->rot(attacker, amount, immediate, true))
    {
        if (defender->is_monster() && defender_visible)
        {
            special_damage_message =
                make_stringf(
                    amount > 0 ? gettext("%s rots!") :
                                 gettext("%s looks less resilient!"),
...
bool melee_attack::distortion_affects_defender()
{
    ...
    {
        special_damage_message =
            make_stringf(_("Space bends around %s."),
            def_name(DESC_THE).c_str());
...
void melee_attack::chaos_affects_defender()
{
    ...
    if (attacker == defender)
    {
        ...
        {
            if (defender->is_player())
                mpr(_("You give off a flash of multicoloured light!"));
...
void melee_attack::chaos_affects_attacker()
{
    ...
    {
        mprf(gettext("Smoke pours forth from %s!"), wep_name(DESC_YOUR).c_str());
        ...
        && one_chance_in(200))
    {
        ...
        if (!you.can_see(attacker))
        {
            ...
            if (!noise.empty())
                msg = make_stringf(_("You hear %s"), 
                        maybe_pick_random_substring(noise).c_str());
        }
        else
        {
...
bool melee_attack::apply_damage_brand()
{
    ...
    case SPWPN_ELECTROCUTION:
        ...
        else if (one_chance_in(3))
        {
            special_damage_message =
                defender->is_player()?
                   _("You are electrocuted!")
                :  _("There is a sudden explosion of sparks!");
                                   ...
                                   brand_type wpn_brand)
{
    ...
    else
    {
        ...
        if (defender->holiness() == MH_NATURAL)
        {
            ...
            {
                if (defender_visible)
                    mpr(_("The flame cauterises the wound!"));
                return true;
            }
...
void melee_attack::apply_staff_damage()
{
    ...
    case STAFF_AIR:
        ...
        {
            special_damage_message =
                /// 1. defender name, 2. are or is
                make_stringf(_("%s %s electrocuted!"),
                             defender->name(DESC_THE).c_str(),
    ...
    case STAFF_DEATH:
        ...
        if (special_damage)
        {
            special_damage_message =
                make_stringf(
                    /// 1. defender name, 2. "" or "s"
                    _("%s convulse%s in agony!"),
                    defender->name(DESC_THE).c_str(),
...
void melee_attack::announce_hit()
{
    ...
    else
    {
        ...
        mprf(gettext("You %s %s%s%s%s"),
             attacker->conj_verb(attack_verb).c_str(),
             defender->name(DESC_THE).c_str(),
...
void melee_attack::splash_defender_with_acid(int strength)
{
    ...
    else
    {
        ...
        if (defender_visible)
            mprf(gettext("%s is splashed with acid."), defender->name(DESC_THE).c_str());
...
void melee_attack::emit_foul_stench()
{
        ...
        && adjacent(you.pos(), mon->pos()))
    {
        ...
        {
            mpr(gettext("You emit a cloud of foul miasma!"));
...
void melee_attack::do_minotaur_retaliation()
{
    ...
    if (!defender->is_player())
    {
        ...
        if (you.see_cell(defender->pos()))
        {
            ...
            mprf(_("%s furiously retaliates!"), defname.c_str());
            if (hurt <= 0)
            {
                mprf(gettext("%s headbutts %s, but does no damage."), defname.c_str(),
                     attacker->name(DESC_THE).c_str());
            }
            else
            {
                mprf(gettext("%s headbutts %s%s"), defname.c_str(),
                     attacker->name(DESC_THE).c_str(),
    ...
    if (5 * you.strength() + 7 * you.dex() > random2(600))
    {
        ...
        mpr(_("You furiously retaliate!"));
        dprf(DIAG_COMBAT, "Retaliation: dmg = %d hurt = %d", dmg, hurt);
        if (hurt <= 0)
        {
            mprf(gettext("You headbutt %s, but do no damage."),
                 attacker->name(DESC_THE).c_str());
            return;
        }
        else
        {
            mprf(gettext("You headbutt %s%s"),
                 attacker->name(DESC_THE).c_str(),
                 get_exclams(hurt).c_str());
            attacker->hurt(&you, hurt);
        }
                                               ...
                                               bool needs_bite_msg, int reduction)
{
    ...
    if (you.hp < you.hp_max)
    {
        ...
        {
            inc_hp(heal);
            mprf(gettext("You feel %sbetter."), (you.hp == you.hp_max) ? gettext("much ") : "");
        }
    }
...
void melee_attack::stab_message()
{
    ...
    case 6:     // big melee, monster surrounded/not paying attention
        if (coinflip())
        {
            mprf((you.species == SP_FELID) ? gettext("You pounce on %s from a blind spot!") :
    ...
    case 4:     // confused/fleeing
        ...
        {
            mprf((you.species == SP_FELID) ? gettext("You pounce on %s from behind!") :
                                             gettext("You strike %s from behind!"),
                  defender->name(DESC_THE).c_str());
        }
        break;
    case 2:
