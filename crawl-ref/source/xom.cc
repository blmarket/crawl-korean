...
static int _xom_throw_divine_lightning(bool debug = false)
{
    ...
    beam.target       = you.pos();
    beam.name         = M_("blast of lightning"); //blast of lightning
...
bool move_stair(coord_def stair_pos, bool away, bool allow_under)
{
    ...
    std::string stair_str =
        feature_description_at(true, stair_pos, false, DESC_PLAIN, false);
...
static int _xom_colour_smoke_trail(bool debug = false)
{
    ...
    const std::string speech = _get_xom_speech(_(M_("colour smoke trail")));
    god_speaks(GOD_XOM, speech.c_str());
