...
void you_teleport(void)
{
    ...
    else
    {
        ...
        {
            mpr(_("You feel the power of the Abyss delaying your translocation."));
                             ...
                             bool wizard_tele, int range)
{
    ...
    if (!is_controlled)
    {
        ...
        if (tries == 0)
            ...
            mpr(_("Your surroundings flicker for a moment."));
...
spret_type cast_apportation(int pow, bolt& beam, bool fail)
{
    ...
    {
        mpr(_("You cannot apport the Orb!"));
        return SPRET_ABORT;
    }
    ...
    if (max_units <= 0)
    {
        ...
        mpr(_("The mass is resisting your pull."));
...
spret_type cast_golubrias_passage(const coord_def& where, bool fail)
{
    ...
    if (tries >= 100 || tries2 >= 100)
    {
        if (you.trans_wall_blocking(randomized_where))
            ...
            mpr(gettext("Creating passages of Golubria requires sufficient empty space."));
        return SPRET_ABORT;
    }
