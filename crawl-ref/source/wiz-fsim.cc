...
static string _equipped_weapon_name()
{
    ...
    if (iweap)
    {
        string item_buf = iweap->name(false, DESC_PLAIN, true);
...
static bool _fsim_kit_equip(const string &kit)
{
    ...
    if (!weapon.empty())
    {
        for (int i = 0; i < ENDOFPACK; ++i)
        {
            ...
            if (you.inv[i].name(false, DESC_PLAIN).find(weapon) != string::npos)
            {
    ...
    if (!missile.empty())
    {
        for (int i = 0; i < ENDOFPACK; ++i)
        {
            ...
            if (you.inv[i].name(false, DESC_PLAIN).find(missile) != string::npos)
            {
                quiver_item(i);
                break;
            }
