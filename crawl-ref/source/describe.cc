...
static std::string _describe_demon(const std::string& name, flight_type fly)
{
	...
	std::string temp_body_descs = RANDOM_ELEMENT(body_descs); std::string temp_wing_names = RANDOM_ELEMENT(wing_names); std::string temp_lev_names = RANDOM_ELEMENT(lev_names); std::string temp_nonfly_names = RANDOM_ELEMENT(nonfly_names); std::string temp_misc_desc = RANDOM_ELEMENT(misc_descs);
    ...
    {
    case FL_FLY:
        description << gettext(temp_wing_names.c_str());
        break;

    case FL_LEVITATE:
        description << gettext(temp_lev_names.c_str());
        break;

    case FL_NONE:  // does not fly
        if (!one_chance_in(4))
            description << gettext(temp_nonfly_names.c_str());
        break;
    }
...
{
    std::string prompt = pgettext("_actions_prompt","You can ");
    int keyin;
    ...
    if (act_str_init)
    {
        ...
        act_str[CMD_MAKE_NOTE]          = _("(a)utoinscribe");
...
static std::string _monster_stat_description(const monster_info& mi)
{
    ...
    {
        const std::string tmp = make_stringf(gettext("immune to %s"),
    ...
    {
        const std::string tmp = make_stringf(gettext("very resistant to %s"),
    ...
    if (mi.fly != FL_NONE)
    {
        result << pronoun << "은(는) " //result << pronoun << " can "
...
static std::string _describe_favour(god_type which_god)
{
    ...
    const std::string godname = _(god_name(which_god).c_str());
...
static std::string _religion_help(god_type god)
{
    ...
    case GOD_VEHUMET:
        ...
        {
            result += std::string(_(god_name(god).c_str())) + gettext(" assists you in casting "
