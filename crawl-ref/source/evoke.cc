            ...
            && !midmons->submerged())
        {
            ...
            if (coinflip())
            {
                ...
                if (mons->wont_attack())
                {
                    ...
                    mpr(_("You could not reach far enough!"));
                    you.time_taken = attack_delay;
                    make_hungry(3, true);
                    return true;
                }
...
string manual_skill_names(bool short_text)
{
    ...
    {
        char buf[40];
        sprintf(buf, _("%lu skills"), (unsigned long) skills.size());
        return string(buf);
    }
    else
