...
static bool _swap_monsters(monster* mover, monster* moved)
{
    ...
    {
        mprf(gettext("%s and %s swap places."), mover->name(DESC_THE).c_str(),
             moved->name(DESC_THE).c_str());
    }
...
static bool _handle_scroll(monster* mons)
{
    ...
    case SCR_TELEPORTATION:
        if (!mons->has_ench(ENCH_TP) && !mons->no_tele(true, false))
        {
            ...
            {
                simple_monster_message(mons, _(" reads a scroll."));
                read = true;
                ident = ID_KNOWN_TYPE;
    ...
    case SCR_SUMMONING:
        ...
        {
            simple_monster_message(mons, _(" reads a scroll."));
            mprf(_("Wisps of shadow swirl around %s."), mons->name(DESC_THE).c_str());
            read = true;
...
{
    if (!simple_monster_message(mons, _(" zaps a wand.")))
    {
        if (!silenced(you.pos()))
            mpr(_("You hear a zap."), MSGCH_SOUND);
    }
...
void handle_monster_move(monster* mons)
{
    ...
    {
        simple_monster_message(mons, _(" is lost in a daze."));
        mons->speed_increment -= non_move_energy;
        return;
    }
...
static bool _jelly_divide(monster* parent)
{
    ...
    *child = *parent;
    ...
    if (!simple_monster_message(parent, _(" splits in two!"))
        && (player_can_hear(parent->pos()) || player_can_hear(child->pos())))
    {
        mpr(_("You hear a squelching noise."), MSGCH_SOUND);
    }
                                       ...
                                       bool do_heal, bool nearby)
{
    ...
    if (nearby)
    {
        mprf(gettext("%s eats %s."), mons->name(DESC_THE).c_str(),
...
static bool _monster_move(monster* mons)
{
    ...
    if (mons->has_ench(ENCH_AQUATIC_LAND))
    {
        ...
        if (adj_move.empty())
        {
            simple_monster_message(mons, _(" flops around on dry land!"));
            return false;
        }
            ...
            if (flattens_trees)
            {
                ...
                if (you.see_cell(target))
                {
                    ...
                    mprf(gettext("%s knocks down a tree!"),
                         actor_visible?
                         mons->name(DESC_THE).c_str() : gettext(M_("Something")));
                    noisy(25, target);
                }
                else
