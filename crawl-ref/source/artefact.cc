...
std::string make_random_kr_name();
...
static bool _pick_db_name(const item_def &item)
{
    ...
    case OBJ_WEAPONS:
    case OBJ_ARMOUR:
        return !(one_chance_in(4)); // coinflip(); (130218, deceit) 0.9와 마찬가지로 한글판의 경우 make_name()을 사용하지 않고 make_random_kr_name()을 사용하는데, 후자의 경우 경우의 수가 한정적이므로, 전자쪽에 무게를 더 두었습니다.
    case OBJ_JEWELLERY:
        return coinflip(); // one_chance_in(5); (deceit, 130218) 무기/방어구는 1/2 -> 3/4 , 장신구는 1/5 -> 1/2.
    default:
        return false;
    }
}
...
std::string make_artefact_name(bool allow_translate, const item_def &item, bool appearance)
{
    ASSERT(is_artefact(item));
    ...
    {
        result += check_gettext(item_base_name(item).c_str()) + std::string(" ");
		...
		{
			if (name.find_first_of("_SUFFIX_") != -1)
			{
			...
			{
				if (name.find_first_of("_NOPOS_") != -1)
				{
    ...
    {
		const std::string st_p = make_random_kr_name();
		result += check_gettext(item_base_name(item).c_str());
