    ...
    if (what)
    {
        ...
        case NOTE_LEARN_SPELL:
            result << spell_difficulty(static_cast<spell_type>(first))     // << "Learned a level "
                   ...
                   << gettext(spell_title(static_cast<spell_type>(first))) // << " spell: "
                   << "을(를) 기억.";                                      // << gettext(spell_title(static_cast<spell_type>(first)));
            break;
        case NOTE_GET_GOD:
        ...
        case NOTE_MOLLIFY_GOD:
            result // << "Was forgiven by "
                   << _(god_name(static_cast<god_type>(first)).c_str()) << "이(가) 화를 누그러트림.";
            break;
        case NOTE_GOD_GIFT:
        ...
        case NOTE_MESSAGE:
            ...
            break;
        case NOTE_SEEN_FEAT:
            result << _(name.c_str()) << "을(를) 발견."; // "Found " << name;
            break;
        case NOTE_FEAT_MIMIC:
            result << _(name.c_str()) << "은(는) 미믹이었음."; // name <<" was a mimic.";
            break;
