...
void pikel_band_neutralise()
{
    ...
    string final_msg;
    if (visible_slaves == 1)
        final_msg = _("With Pikel's spell broken, the former slave thanks you for freedom.");
    else if (visible_slaves > 1)
        final_msg = _("With Pikel's spell broken, the former slaves thank you for their freedom.");
...
void hogs_to_humans()
{
    ...
    {
        if (any == human)
            final_msg = _("No longer under Kirke's spell, the hog turns into a human!");
        else
            final_msg = _("No longer under Kirke's spell, the hog returns to its "
                        "original form!");
    }
    else if (any > 1)
    {
        if (any == human)
            final_msg = _("No longer under Kirke's spell, the hogs revert to their "
                        "human forms!");
        else
            final_msg = _("No longer under Kirke's spell, the hogs revert to their "
                        "original forms!");
