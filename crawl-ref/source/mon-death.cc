...
void pikel_band_neutralise()
{
    ...
    if (visible_slaves == 1)
        mpr(gettext("With Pikel's spell broken, the former slave thanks you for freedom."));
    else if (visible_slaves > 1)
        mpr(gettext("With Pikel's spell broken, the former slaves thank you for their freedom."));
...
void hogs_to_humans()
{
    ...
    {
        if (any == human)
            mpr(_("No longer under Kirke's spell, the hogs revert to their "
                "human forms!"));
        else
            mpr(_("No longer under Kirke's spell, the hogs revert to their "
                "original forms!"));
