...
static bool _have_corpses_in_pack(bool remind, bool bottle_blood = false)
{
    ...
    string verb = (bottle_blood ? pgettext("food","bottle") : pgettext("food","butcher"));
...
bool butchery(int which_corpse, bool bottle_blood)
{
    if (you.visible_igrd(you.pos()) == NON_ITEM)
    {
        if (!_have_corpses_in_pack(false, bottle_blood))
            mpr(_("There isn't anything here!"));
        return false;
    }
    ...
    if (!can_butcher && you.berserk())
    {
        ...
        mpr(_("You are too berserk to search for a butchering tool!"));
        return false;
    }
    ...
    if (you_foodless(true))
    {
        if (!silent)
        {
            if (you.form == TRAN_TREE)
                mpr(_("Just photosynthesize."));
            else if (you.form == TRAN_FUNGUS)
                mpr(_("You decompose your surroundings."));
            else
                mpr(_("You can't eat."));
        ...
        if (unusable_corpse)
        {
            if (you.species == SP_VAMPIRE)
            {
                mprf(gettext("%s devoid of blood."),
                     unusable_corpse == 1 ? gettext("This corpse is")
                                          : gettext("These corpses are"));
            }
            else
...
static void _eat_chunk(item_def& food)
{
    ...
    {
    case CE_MUTAGEN:
        mpr(_("This meat tastes really weird."));
        mutate(RANDOM_MUTATION, M_("mutagenic meat"));
...
void vampire_nutrition_per_turn(const item_def &corpse, int feeding)
{
    ...
    switch (chunk_type)
    {
        case CE_CLEAN:
            if (start_feeding)
            {
                mprf(gettext("This %sblood tastes delicious!"),
                     mons_class_flag(mons_type, M_WARM_BLOOD) ? pgettext("blood", "warm ")
                                                              : "");
            }
        ...
        case CE_MUTAGEN:
            food_value /= 2;
            if (start_feeding)
                mpr(_("This blood tastes really weird!"));
            mutate(RANDOM_MUTATION, M_("mutagenic blood"));
            did_god_conduct(DID_DELIBERATE_MUTATING, 10);
            xom_is_stimulated(100);
...
bool can_ingest(const item_def &food, bool suppress_msg, bool check_hunger)
{
    ...
    {
        if (!suppress_msg)
            mpr(_("Eating this would be a bad idea."));
        return false;
    }
                ...
                bool check_hunger, bool rotten)
{
    ...
    case OBJ_FOOD:
    {
        ...
        else if (food_is_meaty(kindof_thing))
        {
            ...
            {
                if (!suppress_msg)
                    mpr(_("Sorry, you're a herbivore."));
                return false;
            }
...
static bool _vampire_consume_corpse(int slot, bool invent)
{
    ...
    {
        mpr(_("There is no blood in this body!"));
        return false;
    }

    if (food_is_rotten(corpse))
    {
        mpr(_("It's not fresh enough."));
        return false;
    }
