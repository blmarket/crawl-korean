...
#define SAY(x) {if (say_reason) { x; }}
    ...
    {
        SAY(mpr(_("Your weapon is melded into your body!")));
        return false;
    }

    if (!ignore_temporary_disability && !form_can_wield(you.form))
    {
        SAY(mpr(_("You can't wield anything in your present form.")));
        return false;
    }
    ...
    if (!ignore_temporary_disability
        ...
        && you.weapon()->cursed())
    {
        SAY(mprf(_("You can't unwield your weapon%s!"),
                 !unwield ? _(" to draw a new one") : ""));
        return false;
    }
    ...
    for (int i = EQ_MIN_ARMOUR; i <= EQ_MAX_WORN; i++)
    {
        ...
        {
            SAY(mpr(_("You are wearing that object!")));
            return false;
        }
    }
    ...
    {
        SAY(mpr(_("That's too large and heavy for you to wield.")));
        return false;
    }
    ...
    if (weapon->base_type != OBJ_WEAPONS)
    {
        ...
        {
            SAY(mpr(_("You can't wield that with a shield.")));
            return false;
        }
        else
            return true;
    ...
    {
        SAY(mpr(_("That's too large for you to wield.")));
        return false;
    }
                  ...
                  bool force, bool show_unwield_msg, bool show_wield_msg)
{
    ...
    {
        mpr(_("You are already wielding that!"));
        return true;
    }
...
bool can_wear_armour(const item_def &item, bool verbose, bool ignore_temporary)
{
    ...
    if (sub_type == ARM_GLOVES)
    {
        ...
        {
            if (verbose)
                mpr(_("You can't wear gloves with your huge claws!"));
            return false;
        }
    }
    ...
    if (sub_type == ARM_BOOTS)
    {
        ...
        {
            if (verbose)
                mpr(_("You can't wear boots with hooves!"));
            return false;
        }
    ...
    if (slot == EQ_HELMET)
    {
        ...
        {
            if (verbose)
                mpr(_("You can't wear any headgear with your large horns!"));
            return false;
        }
        ...
        if (is_hard_helmet(item))
        {
            ...
            {
                if (verbose)
                    mpr(_("You can't wear that with your horns!"));
                return false;
            }
            ...
            {
                if (verbose)
                    mpr(_("You can't wear that with your beak!"));
                return false;
            }
            ...
            {
                if (verbose)
                    mpr(_("You can't wear that with your antennae!"));
                return false;
            }
            ...
            {
                if (verbose)
                    mpr(_("You can't wear that with your reptilian head."));
                return false;
            }
...
//    mprf("Wearing %s.", you.inv[new_ring].name(DESC_A).c_str());
    ...
    mprf(MSGCH_PROMPT,
         _("You're wearing all the rings you can. Remove which one?"));
...
static bool _swap_rings(int ring_slot)
{
    ...
    {
        mprf(_("You're already wearing two cursed rings!"));
        return false;
    }
...
static bool _swap_rings_octopode(int ring_slot)
{
    ...
    {
        mprf(_("You're already wearing %s cursed rings!%s"),
             number_in_words(cursed).c_str(),
             (cursed == 8 ? _(" Isn't that enough for you?") : ""));
        return false;
    }
...
static bool _puton_item(int item_slot)
{
    ...
    if (item.base_type != OBJ_JEWELLERY)
    {
        mpr(_("You can only put on jewellery."));
        return false;
    }
    ...
    if (!is_amulet)     // i.e. it's a ring
    {
        ...
        {
            mpr(_("You can't take your gloves off to put on a ring!"));
            return false;
        }
...
bool remove_ring(int slot, bool announce)
{
    ...
    {
        if (has_melded)
            mpr(_("You aren't wearing any unmelded rings or amulets."));
        else
            mpr(_("You aren't wearing any rings or amulets."));
    ...
    {
        mpr(_("You can't take your gloves off to remove any rings!"));
        return false;
    }
    ...
    if (hand_used == EQ_NONE)
    {
        ...
        {
            mpr(_("That isn't a piece of jewellery."));
            return false;
        }
    }
    ...
    {
        mpr(_("I don't think you really meant that."));
        return false;
    }
    else if (you.melded[hand_used])
    {
        mpr(_("You can't take that off while it's melded."));
        return false;
    }
    ...
    mprf(_("You remove %s."), you.inv[ring_wear_2].name(true, DESC_YOUR).c_str());
#ifdef USE_TILE_LOCAL
    const unsigned int old_talents = your_talents(false).size();
#endif
...
static bool _dont_use_invis()
{
    ...
    {
        mpr(_("You can't turn invisible."));
        return true;
    }
...
static bool _drink_fountain()
{
    ...
    {
        if (!yesno(_("Drink from the fountain of blood?"), true, 'n'))
            return false;
    ...
    {
        if (!yesno(_("Drink from the sparkling fountain?"), true, 'n'))
            return false;
...
static void _rebrand_weapon(item_def& wpn)
{
    ...
    const string itname = wpn.name(true, DESC_YOUR); //wpn.name(DESC_YOUR);
...
static void _brand_weapon(bool alreadyknown, item_def &wpn)
{
    ...
    case SPWPN_EVASION:
        ...
        {
            alert_nearby_monsters();
            mprf(_("%s emits a brilliant flash of light!"),itname.c_str());
        }
        else // should be VORPAL only
        {
            if (get_vorpal_type(wpn) != DVORP_CRUSHING)
                mprf(_("%s's sharpness seems more permanent."), itname.c_str());
            else
                mprf(_("%s's heaviness feels very stable."), itname.c_str());
        }
        break;

    case SPWPN_FLAME:
    case SPWPN_FLAMING:
        mprf(_("%s is engulfed in an explosion of fire!"), itname.c_str());
        immolation(10, IMMOLATION_AFFIX, alreadyknown);
        break;

    case SPWPN_FROST:
    case SPWPN_FREEZING:
        mprf(_("%s is covered with a thin layer of ice!"), itname.c_str());
    ...
    case SPWPN_ELECTROCUTION:
        mprf(_("%s releases a massive orb of lightning."), itname.c_str());
        _explosion(you.pos(), &you, BEAM_ELECTRICITY, LIGHTCYAN, "electricity",
                   "electrocution affixation");
        break;

    case SPWPN_CHAOS:
        mprf(_("%s erupts in a glittering mayhem of all colours."), itname.c_str());
    ...
    case SPWPN_HOLY_WRATH:
        mprf(_("%s emits a blast of cleansing flame."), itname.c_str());
...
void read_scroll(int slot)
{
    ...
    if (you.stat_zero[STAT_INT] && !one_chance_in(5))
    {
        ...
        mpr(_("You almost manage to decipher the scroll, but fail in this attempt."));
        return;
    }
    ...
    case SCR_IMMOLATION:
    {
        ...
        if (had_effect)
            mpr(_("The creatures around you are filled with an inner flame!"));
        else
            mpr(_("The air around you briefly surges with heat, but it dissipates."));
    ...
    case SCR_BRAND_WEAPON:
        ...
        {
            mpr(pre_succ_msg.c_str());
            mpr(_("It is a scroll of brand weapon."));
