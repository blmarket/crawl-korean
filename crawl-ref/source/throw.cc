    ...
    else
    {
        ...
        if (returning && item_type_known(you.inv[throw_2]))
        {
            msg::stream << item.name(true, DESC_THE)
                        << _(" fails to return to your pack!") << endl;
        }
...
bool mons_throw(monster* mons, bolt &beam, int msl)
{
    ...
    string msg = mons->name(DESC_THE) + "은(는) ";
    ...
    if (really_returns)
    {
        ...
        {
            msg::stream << "그 무기는 "
                        <<  (you.can_see(mons) ? mons->name(DESC_PLAIN) + "에게로"
                            : "왔던 곳으로")
                        << "되돌아갔다." << std::endl;
...
bool thrown_object_destroyed(item_def *item, const coord_def& where)
{
    ...
    string name = item->name(true, DESC_PLAIN, false, true, false);
