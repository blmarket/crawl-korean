...
void monster::remove_enchantment_effect(const mon_enchant &me, bool quiet)
{
    ...
    case ENCH_INVIS:
        ...
        {
            if (!quiet)
                mprf(_("%s appears from thin air!"), name(DESC_A, true).c_str());
    ...
    case ENCH_SUBMERGED:
        ...
        if (you.can_see(this))
        {
            ...
            else if (!quiet)
            {
                ...
                if (type == MONS_AIR_ELEMENTAL)
                {
                    mprf(channel, _("%s forms itself from the air!"),
                                  name(DESC_PLAIN, true).c_str());
                }
                else if (type == MONS_TRAPDOOR_SPIDER)
                {
...
void monster::apply_enchantment(const mon_enchant &me)
{
    ...
    case ENCH_STICKY_FLAME:
    {
              ...
              || mons_intel(this) >= I_NORMAL && flight_mode()))
        {
            if (mons_near(this) && visible_to(&you))
            {
                mprf(ground_level() ? _("The flames covering %s go out.")
                     : _("%s dips into the water, and the flames go out."),
        ...
        if (decay_enchantment(en))
        {
            if (you.can_see(this))
            {
                if (type == MONS_PILLAR_OF_SALT)
                    mprf(_("%s crumbles away."), name(DESC_THE, false).c_str());
                else
                {
    ...
    case ENCH_SILVER_CORONA:
        if (is_chaotic())
        {
            ...
            if (newdam > 0)
            {
                string msg = mons_has_flesh(this) ? _("'s flesh") : "";
