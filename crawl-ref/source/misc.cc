                 ...
                 bool check_dist, int range)
{
    ...
    if (visible.size() == 1)
    {
        ...
        const string monname = mons_is_mimic(m.type) ? _(M_("A mimic"))
                                                     : m.name(DESC_THE);
        msg = make_stringf(_("%s is nearby!"), monname.c_str());
    }
...
void timeout_terrain_changes(int duration, bool force)
{
    ...
    if (num_seen[TERRAIN_CHANGE_DOOR_SEAL] > 1)
        mpr(_("The runic seals fade away."));
    else if (num_seen[TERRAIN_CHANGE_DOOR_SEAL] > 0)
        mpr(_("The runic seal fades away."));
}
...
void auto_id_inventory()
{
    for (int i = 0; i < ENDOFPACK; i++)
    {
        ...
        if (item.defined())
        {
            maybe_id_weapon(item, _("You determine that you are carrying: "));
...
void entered_malign_portal(actor* act)
{
    if (you.can_see(act))
    {
        mprf(_("The portal repels %s, its terrible forces doing untold damage!"),
             act->is_player() ? gettext(M_("you")) : act->name(DESC_THE).c_str());
    }
