    ...
    if (!moves.isCancel && moves.isTarget)
    {
        ...
        if (looking_at_you())
        {
                                     ...
                                     || mode == TARG_HOSTILE_SUBMERGED))
            {
                ...
                {
                    mpr(_("That would be overly suicidal."), MSGCH_EXAMINE_FILTER);
                    return false;
                }
...
static void _describe_oos_feature(const coord_def& where)
{
    ...
    if (feat == DNGN_SECRET_DOOR)
        ...
        desc = feature_description_at(true, where);
...
std::string feature_description_kr(dungeon_feature_type grid,
                                ...
                                const std::string & cover_desc,
                                description_level_type dtype,
                                bool add_stop, bool base_desc) // (130223, deceit) 번역된 trap명을 받기 위해 따로 함수를 하나 더 만듬.
{
    std::string desc = _(_base_feature_desc(grid, trap).c_str());
...
std::string feature_description_at(bool allow_translate, const coord_def& where, bool covering,
...
{	// (130223,deceit) feature name쪽도 item name쪽처럼 allow_translate인자를 추가하였습니다. 0.9때는 처음부터 번역명을 써서 지형지물 검색쪽에 문제가 생겼죠
    ...
    if (grid == DNGN_OPEN_DOOR || feat_is_closed_door(grid))
    {
        ...
        if (!allow_translate) desc = make_stringf("%s%s", desc.c_str(), covering_description.c_str()); else desc = make_stringf("%s%s", covering_description.c_str(), desc.c_str());
...
static std::string _get_monster_desc(const monster_info& mi)
{
    ...
    if (mi.is(MB_SLEEPING) || mi.is(MB_DORMANT))
    {
        text += make_stringf(_("%s appears to be %s.\n"), pronoun.c_str(),
                (mi.is(MB_CONFUSED) ? _("sleepwalking")
                        : _("resting")));
    ...
    if (mi.is(MB_SUPPRESSED))
        text += pronoun + _(" exudes an aura of magical suppression.\n");
    ...
    if (mi.fire_blocker)
    {
        text += make_stringf(_("Your line of fire to %s is blocked by %s\n"), mi.pronoun(PRONOUN_OBJECTIVE), // "std::string("Your line of fire to ") + mi.pronoun(PRONOUN_OBJECTIVE)
...
#define no_warn(x) (!item_type_known(*x) || !item_is_branded(*x))
        ...
        {
            if (found_sth)
                desc += (!mon_rng && !mon_qvr && !mon_carry) ? _(" and") : ",";
            else
                found_sth = true;
...
static void _debug_describe_feature_at(const coord_def &where)
{
    const std::string feature_desc = feature_description_at(false, where, true);
    std::string marker;
...
static void _describe_cell(const coord_def& where, bool in_range)
{
  ...
  look_clouds:
    ...
    std::string feature_desc = feature_description_at(false, where, true);
