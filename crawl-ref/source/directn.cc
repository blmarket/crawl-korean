...
void full_describe_view()
{
    ...
    title = "Visible " + title;
    string title1 = title + _(" (select to view/travel, '!' to examine):");
    title += _(" (select for more detail, '!' to view/travel):");
    ...
    if (!list_features.empty())
    {
        ...
        for (unsigned int i = 0; i < list_features.size(); ++i, hotkey++)
        {
            ...
            desc += "</" + colour_str +">) ";
#endif
            desc += feature_description_at(false, c, false, DESC_A, false);
...
static string _base_feature_desc(dungeon_feature_type grid, trap_type trap)
{
    ...
    case DNGN_RUNED_DOOR:
        return (M_("runed door"));
    case DNGN_SEALED_DOOR:
        return (M_("sealed door"));
    ...
    case DNGN_ABYSSAL_STAIR:
        return (M_("gateway leading deeper into the Abyss"));
    ...
    case DNGN_RETURN_FROM_TOMB:
        if (parent_branch(BRANCH_TOMB) == BRANCH_FOREST)
            return (M_("staircase back to the Forest"));
...
static string _get_monster_desc(const monster_info& mi)
{
    ...
    if (mi.is(MB_SUMMONED) || mi.is(MB_PERM_SUMMON))
    {
        ...
        if (mi.is(MB_SUMMONED_CAPPED))
            text += _(", and is expiring");
        else if (mi.is(MB_PERM_SUMMON))
            text += _(" but will not time out");
        text += ".\n";
    }
