...
static void _explore_find_target_square()
{
    ...
    {
        if (runed_door_pause)
            mpr(_("Partly explored, obstructed by runed door."));
        else
        {
            ...
            else
            {
                ...
                mprf(_("Partly explored, can't reach some %s."),
...
void do_explore_cmd()
{
    if (you.hunger_state == HS_STARVING && !you_min_hunger())
        ...
        mpr(_("Calm down first, please."));
    else if (player_in_branch(BRANCH_LABYRINTH))
        mpr(_("No exploration algorithm can help you here."));
...
void explore_discoveries::add_item(const item_def &i)
{
    item_def copy = i;
    copy.quantity = 1;
    const string cname = copy.name(false, DESC_PLAIN);
    ...
    for (int j = 0, size = items.size(); j < size; ++j)
    {
        ...
        if (cname == items[j].thing.name(false, DESC_PLAIN))
        {
    ...
    const C &coll, const char *category) const
{
    ...
    const string message = make_stringf(gettext("Found %s."), 
