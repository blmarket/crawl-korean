               ...
               bool just_check)
{
    ...
    case TRAN_PIG:
        tran_name = M_("pig");
        msg       = _("You have been turned into a pig!");
        if (!just_check)
    ...
    case TRAN_APPENDAGE:
    {
        ...
        if (!just_check)
        {
            ...
            case MUT_TENTACLE_SPIKE:
                ...
                break;
            case MUT_CLAWS:
                msg = _("Your hands morph into claws.");
                break;
            case MUT_TALONS:
    ...
    case TRAN_DRAGON:
        if (you.attribute[ATTR_HELD])
        {
            ...
            if (trap && trap->type == TRAP_WEB)
            {
                mpr(gettext("You shred the web into pieces!"));
...
void untransform(bool skip_wielding, bool skip_move)
{
    ...
    case TRAN_ICE_BEAST:
        if (you.species == SP_LAVA_ORC && !temperature_effect(LORC_STONESKIN))
            mpr(_("Your icy form melts away into molten rock."), MSGCH_DURATION);
        else
            mpr(_("You warm up again."), MSGCH_DURATION);
