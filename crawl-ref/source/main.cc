...
NORETURN static void _launch_game()
{
    ...
    if (!crawl_state.game_is_tutorial())
        mpr(gettext("Press <w>?</w> for a list of commands and other information."));
...
static void _take_starting_note()
{
    ...
    notestr // << you.your_name << ", the "
            ...
            << _(you.class_name.c_str()) << ", " << you.your_name
...
static void _startup_hints_mode()
{
    ...
    msg::streams(MSGCH_TUTORIAL)
        << _("Press any key to start the hints mode intro, or Escape to skip it.")
        << endl;
                             ...
                             bool known_shaft)
{
    ...
    if (!feat_is_bidirectional_portal(ftype))
    {
            ...
            && (!down || !known_shaft))
        {
            ...
            {
                mpr(_("A magical barricade bars your way!"));
            }
            else if (down)
...
static void _experience_check()
{
    ...
    {
        /// 1. 현재 경험치%, 2. 다음 레벨
        mprf(gettext("You are %d%% of the way to level %d."), perc,
...
static void _print_friendly_pickup_setting(bool was_changed)
{
    ...
    string now = (was_changed? pgettext("_print_friendly_pickup_setting", "now ") : "");
...
void process_command(command_type cmd)
{
    ...
    default:
        if (crawl_state.game_is_hints())
        {
            string msg = _("Unknown command. (For a list of commands type "
                         "<w>?\?<lightgrey>.)");
            mpr(msg);
        }
        else // well, not examine, but...
            mpr(_("Unknown command."), MSGCH_EXAMINE_FILTER);
...
void world_reacts()
{
    ...
    if (you.elapsed_time >= 2*1000*1000*1000)
    {
        ...
        mpr(_("Outside, the world ends."));
        mpr(_("Sorry, but your quest for the Orb is now rather pointless. "
            "You quit..."));
...
static void _move_player(coord_def move)
{
    ...
    if (!attacking && targ_pass && moving && !beholder && !fmonger)
    {
        if (crawl_state.game_is_zotdef() && you.pos() == env.orb_pos)
        {
            ...
            {
                string prompt = _("Are you sure you want to leave the Orb unguarded?");
                if (!yesno(prompt.c_str(), false, 'n'))
                {
                    canned_msg(MSG_OK);
                    return;
