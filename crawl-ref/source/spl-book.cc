                        ...
                        formatted_string *fs)
{
    ...
    {
    case RBOOK_USE_ROD:
        out.cprintf(_("Select a spell to cast.\n"));
        break;
                          ...
                          spell_type current_spell = SPELL_NO_SPELL)
{
    ...
    {
        if (!just_check)
            mpr(_("Your head is already too full of spells!"));
        return false;
    }
...
#else
    spell_menu.set_title(
        new MenuEntry(_("     Spells (Memorisation)         Type          "
                      "                Failure  Level"),
            MEL_TITLE));

    spell_menu.set_title(
        new MenuEntry(_("     Spells (Description)          Type          "
                      "                Failure  Level"),
            MEL_TITLE), false);
#endif
...
bool can_learn_spell(bool silent)
{
    ...
    {
        if (!silent)
            mpr(_("Your brain is not functional enough to learn spells."));
        return false;
    }
...
static bool _learn_spell_checks(spell_type specspell)
{
    ...
    {
        mpr(_("You already know that spell!"));
        return false;
    }
    ...
    {
        mpr(_("You're too inexperienced to learn that spell!"));
        return false;
    }
    ...
    if (!is_artefact(book))
    {
        ...
        if (!book.props.exists(ARTEFACT_APPEAR_KEY))
        {
            book.props[ARTEFACT_APPEAR_KEY].get_string() =
                make_artefact_name(false, book, true); // 일단 randart book은 해석하지 않습니다.
        }
    }
}
