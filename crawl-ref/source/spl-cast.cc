...
static void _surge_power(spell_type spell)
{
    ...
    {
        const string modifier = (enhanced  < -2) ? pgettext("splcast","extraordinarily") :
                                (enhanced == -2) ? pgettext("splcast","extremely") :
                                (enhanced ==  2) ? pgettext("splcast","strong") :
                                (enhanced  >  2) ? pgettext("splcast","huge")
                                                 : "";
        mprf(pgettext("splcast","You feel %s %s"), // 메모
             !modifier.length() ? "a"
                                : article_a(modifier).c_str(),
             (enhanced < 0) ? _("numb sensation.")
                            : _("surge of power!"));
    }
}
...
bool cast_a_spell(bool check_range, spell_type spell)
{
    ...
    if (check_range && spell_no_hostile_in_range(spell))
    {
        ...
        mpr(_("You can't see any susceptible monsters within range! "
...
bool spell_is_uncastable(spell_type spell, string &msg)
{
    ...
    {
        msg = _("You cannot cast that spell in your current form!");
        return true;
    }

    if (_vampire_cannot_cast(spell))
    {
        msg = _("Your current blood level is not sufficient to cast that spell.");
        return true;
    }
                                  ...
                                  bool wiz_cast)
{
    ...
    {
        mpr(_("The dungeon can only cope with one malign gateway at a time!"));
        return true;
    }
    ...
    {
        mpr(_("You need to wait for the winds to calm down."));
        return true;
    }
                       ...
                       bool allow_fail, bool check_range)
{
    ...
    if ((flags & SPFLAG_TARGETTING_MASK) && spell != SPELL_PORTAL_PROJECTILE)
    {
        ...
        if (dir == DIR_DIR)
            mpr(prompt ? prompt : _("Which direction?"), MSGCH_PROMPT);
        ...
        string title = _("Aiming: <white>");
        title += _(spell_title(spell));
        title += "</white>";
        ...
        if (testbits(flags, SPFLAG_NOT_SELF) && spd.isMe())
        {
            if (spell == SPELL_TELEPORT_OTHER)
                mpr(_("Sorry, this spell works on others only."));
            else
    ...
    case SPRET_FAIL:
    {
        ...
        mprf(_("You miscast %s."), _(spell_title(spell)));
        flush_input_buffer(FLUSH_ON_FAILURE);
...
string spell_schools_string(spell_type spell)
{
    ...
    for (int i = 0; i <= SPTYP_LAST_EXPONENT; ++i)
    {
        if (spell_typematch(spell, (1<<i)))
        {
            if (already)
                desc += "/";
            desc += gettext(spelltype_long_name(1 << i));
            already = true;
        }
    }
