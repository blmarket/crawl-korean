...
spret_type cast_delayed_fireball(bool fail)
{
    ...
    {
        mpr(gettext("You are already charged."));
        return SPRET_ABORT;
    }

    fail_check();
    // Okay, this message is weak but functional. - bwr
    mpr(gettext("You feel magically charged."));
...
spret_type cast_fire_storm(int pow, bolt &beam, bool fail)
{
    ...
    {
        mpr(gettext("You can't place the storm on a wall."));
        return SPRET_ABORT;
    }
...
bool cast_hellfire_burst(int pow, bolt &beam)
{
    ...
    mpr(gettext("You call forth a pillar of hellfire!"));
...
spret_type cast_chain_lightning(int pow, const actor *caster, bool fail)
{
         ...
         pow -= 8 + random2(13), source = target)
    {
        ...
        {
            if (see_source)
                mpr(gettext("The lightning grounds out."));
        ...
        const char* msg = gettext("You hear a mighty clap of thunder!");
        noisy(25, source, (first || !see_source) ? msg : NULL);
        first = false;

        if (see_source && !see_targ)
            mpr(gettext("The lightning arcs out of your line of sight!"));
        else if (!see_source && see_targ)
            mpr(gettext("The lightning arc suddenly appears!"));
                                   ...
                                   bool actual, bool added_effects)
{
    ...
    {
        if (actual)
            mpr(_("The light passes straight through your body."));
        levels = 0;
    }
    else if (actual)
    {
                                    ...
                                    int avg, bool actual, bool added_effects)
{
    ...
    else if (you.can_see_invisible())
    {
        if (actual)
        {
            ...
            mprf(_("The light passes through %s."),
                                ...
                                int pow, int total_damage)
{
    if (!affected_monsters.empty())
    {
        ...
        else
        {
            ...
            // affected.
            if (!agent)
                mpr(_("Nearby monsters are poisoned!"));
            else
                mprf(_("The monsters around %s are poisoned!"),
    ...
    if (total_damage)
    {
        ...
        else if (agent)
        {
            ...
            if (mons->heal(total_damage))
                simple_monster_message(mons, _(" is healed."));
        }
    }
}
                                 ...
                                 bool actual, bool added_effects, bool fail)
{
    ...
    switch(spell)
    {
        case SPELL_OZOCUBUS_REFRIGERATION:
            player_msg = _("The heat is drained from your surroundings.");
            global_msg = _("Something drains the heat from around you.");
            mons_vis_msg = _(" drains the heat from the surrounding"
                           " environment!");
            mons_invis_msg = _("The ambient heat is drained!");
        ...
        case SPELL_DRAIN_LIFE:
            player_msg = _("You draw life from your surroundings.");
            global_msg = _("Something draws the life force from your"
                         " surroundings.");
            mons_vis_msg = _(" draws from the surrounding life force!");
            mons_invis_msg = _("The surrounding life force dissipates!");
        ...
        case SPELL_OLGREBS_TOXIC_RADIANCE:
            player_msg = _("You radiate a sickly green light!");
            global_msg = mons_invis_msg =
                _("The air is filled with a sickly green light!");
            mons_vis_msg = _(" radiates a sickly green light!");
            flash_colour = GREEN;
...
void sonic_damage(bool scream)
{
    ...
    */
    ...
    {
        const string message =
            make_stringf(_("%s %s hurt by the noise."),
        ...
        else
        {
            // Exclamation mark to suggest that a lot of creatures were
            // affected.
            mpr(gettext("The monsters around you reel from the noise!"));
        }
    }
...
spret_type vampiric_drain(int pow, monster* mons, bool fail)
{
    ...
    {
        mpr(gettext("Draining that being is not a good idea."));
        return SPRET_ABORT;
    }
    ...
    {
        mpr(gettext("You feel life coursing into your body."));
        inc_hp(hp_gain);
    }
...
spret_type cast_freeze(int pow, monster* mons, bool fail)
{
    ...
    if (!abort && !fail)
    {
        ...
        mprf(gettext("You freeze %s."), mons->name(DESC_THE).c_str());
...
spret_type cast_airstrike(int pow, const dist &beam, bool fail)
{
    ...
    if (mons->res_wind() > 0)
    {
        if (mons->observable())
        {
            mprf(gettext("But air would do no harm to %s!"),
                 mons->name(DESC_THE).c_str());
            return SPRET_ABORT;
        }

        fail_check();
        mprf(gettext("The air twists arounds and harmlessly tosses %s around."),
             mons->name(DESC_THE).c_str());
    ...
    mprf(gettext("The air twists around and strikes %s!"),
         mons->name(DESC_THE).c_str());
    noisy(4, beam.target);
...
static int _shatter_items(coord_def where, int pow, actor *)
{
    ...
    if (broke_stuff)
    {
        if (player_can_hear(where))
            mpr(gettext("You hear glass break."), MSGCH_SOUND);
...
static int _shatter_walls(coord_def where, int pow, actor *agent)
{
    ...
    case DNGN_SEALED_DOOR:
        if (you.see_cell(where))
            mpr(gettext("A door shatters!"));
        chance = 100;
        break;

    case DNGN_GRATE:
        if (you.see_cell(where))
            mpr(gettext("An iron grate is ripped into pieces!"));
        chance = 100;
        break;
...
spret_type cast_shatter(int pow, bool fail)
{
    ...
    if (silence)
        mpr(gettext("The dungeon shakes!"));
    else
    {
        noisy(30, you.pos());
...
static int _ignite_poison_affect_item(item_def& item, bool in_inv)
{
        ...
        && get_weapon_brand(item) == SPWPN_VENOM)
    {
        ...
        {
            mprf(gettext("%s bursts into flame!"),
...
static int _ignite_poison_monsters(coord_def where, int pow, int, actor *actor)
{
    ...
    if (damage > 0)
    {
        ...
        simple_monster_message(mon, gettext(" seems to burn from within!"));
...
static int _ignite_poison_player(coord_def where, int pow, int, actor *actor)
{
    ...
    if (damage)
    {
        ...
        if (resist > 0)
        {
            mpr(gettext("You feel like your blood is boiling!"));
            damage /= 3;
        }
        else if (resist < 0)
        {
            mpr(gettext("The poison in your system burns terribly!"));
            damage *= 3;
        }
        else
            mpr(gettext("The poison in your system burns!"));
        ...
        {
            mpr(gettext("You feel that the poison has left your system."));
...
static int _discharge_monsters(coord_def where, int pow, int, actor *)
{
    ...
    if (where == you.pos())
    {
        mpr(gettext("You are struck by lightning."));
    ...
    else
    {
        ...
        if (damage)
        {
            mprf(gettext("%s is struck by lightning."),
                 mons->name(DESC_THE).c_str());
    ...
    {
        mpr(gettext("The lightning arcs!"));
        pow /= (coinflip() ? 2 : 3);
...
spret_type cast_discharge(int pow, bool fail)
{
    ...
    {
        if (coinflip())
            mpr(gettext("The air around you crackles with electrical energy."));
        else
        {
            const bool plural = coinflip();
            mprf(gettext("%s blue arc%s ground%s harmlessly %s you."),
                 plural ? pgettext("spl-damage","Some") : pgettext("spl-damage","A"),
                 plural ? "s" : "",
                 plural ? pgettext("spl-damage"," themselves") : pgettext("spl-damage","s itself"),
                 plural ? pgettext("spl-damage","around") : (coinflip() ? pgettext("spl-damage","beside") :
                                      coinflip() ? pgettext("spl-damage","behind") : pgettext("spl-damage","before")));
        }
    }
    return SPRET_SUCCESS;
}
...
spret_type cast_dispersal(int pow, bool fail)
{
    ...
    if (!apply_monsters_around_square(_disperse_monster, you.pos(), pow))
        mpr(gettext("The air shimmers briefly around you."));
                                ...
                                "veto_fragmentation") == "veto")
    {
        if (caster->is_player() && !quiet)
        {
            mprf(_("%s seems to be unnaturally hard."),
                 feature_description_at(true, target, false, DESC_THE, false).c_str());
        }
        return false;
    }
                              ...
                              const coord_def target, bool fail)
{
    ...
    {
        if (you.see_cell(target))
            mprf(_("%s shatters!"), _(mon->name(DESC_THE).c_str()));
...
void forest_damage(const actor *mon)
{
    ...
    if (one_chance_in(4))
        forest_message(pos, gettext(random_choose(
            N_("The trees move their gnarly branches around."),
            N_("You feel roots moving beneath the ground."),
            N_("Branches wave dangerously above you."),
            N_("Trunks creak and shift."),
            N_("Tree limbs sway around you."),
            0)), MSGCH_TALK_VISUAL);
    ...
    for (radius_iterator ri(pos, LOS_RADIUS); ri; ++ri)
    {
        ...
        for (adjacent_iterator ai(*ri); ai; ++ai)
            if (feat_is_tree(grd(*ai)) && cell_see_cell(pos, *ai, LOS_DEFAULT))
            {
                ...
                {
                    msg = _(random_choose(
                            N_("@foe@ @is@ waved at by a branch."),
                            N_("A tree reaches out but misses @foe@."),
                            N_("A root lunges up near @foe@."),
                            0));
                }
                else if (!apply_chunked_AC(1, foe->melee_evasion(mon) - evnp))
                {
                    msg = _(random_choose(
                            N_("A branch passes through @foe@!"),
                            N_("A tree reaches out and and passes through @foe@!"),
                            N_("A root lunges and passes through @foe@ from below."),
                            0));
                }
                ...
                {
                    msg = _(random_choose(
                            N_("@foe@ @is@ scraped by a branch!"),
                            N_("A tree reaches out and scrapes @foe@!"),
                            N_("A root barely touches @foe@ from below."),
                            0));
                }
                else
                {
                    msg = _(random_choose(
                        N_("@foe@ @is@ hit by a branch!"),
                        N_("A tree reaches out and hits @foe@!"),
                        N_("A root smacks @foe@ from below."),
                        0));
                }
