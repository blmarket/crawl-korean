...
void banished(const string &who)
{
    ...
    if (player_in_branch(BRANCH_ABYSS))
    {
        ...
        else
        {
            ...
            mpr(_("You are banished to a different region of the Abyss."), MSGCH_BANISHMENT);
            abyss_teleport(true);
        }
        return;
    }
...
void handle_time()
{
    ...
    if (int lev = player_mutation_level(MUT_EVOLUTION))
               ...
               > (int)exp_needed(you.experience_level + 1))
        {
            ...
            mpr(_("You feel a genetic drift."));
...
bool mushroom_spawn_message(int seen_targets, int seen_corpses)
{
    if (seen_targets > 0)
    {
        string what  = seen_targets  > 1 ? _("Some toadstools")
                                         : _("A toadstool");
        string where = seen_corpses  > 1 ? _("nearby corpses") :
                       seen_corpses == 1 ? _("a nearby corpse")
                                         : _("the ground");
        mprf(_("%s grow%s from %s."),
...
void slime_wall_damage(actor* act, int delay)
{
    ...
    else
    {
        ...
        {
            mprf((walls > 1) ? _("The walls burn %s!") : _("The wall burns %s!"),
