...
static void _wield_cursed(item_def& item, bool known_cursed, bool unmeld)
{
    if (!item.cursed() || unmeld)
        return;
    mprf(gettext("It sticks to your %s!"), gettext(you.hand_name(false).c_str()));
    int amusement = 16;
    if (!known_cursed)
    {
        amusement *= 2;
            ...
            if (showMsgs)
            {
                ...
                case SPWPN_PAIN:
                {
                    ...
                    if (you.skill(SK_NECROMANCY) == 0)
                        ...
                        mprf(gettext("Pain shudders through your %s!"), gettext(your_arm));
                    else
                        mprf(gettext("A searing pain shoots up your %s!"), gettext(your_arm));
                    break;
                }
...
static void _equip_jewellery_effect(item_def &item, bool unmeld)
{
    ...
    case RING_TELEPORTATION:
        if (crawl_state.game_is_sprint())
            ...
            mprf(gettext("You feel slightly %sjumpy."),
                 (player_teleport(false) > 8) ? pgettext("teleport", "more ") : "");
        ident = ID_KNOWN_TYPE;
        break;
