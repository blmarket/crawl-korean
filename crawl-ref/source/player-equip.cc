...
static void _unequip_weapon_effect(item_def& item, bool showMsgs, bool meld)
{
    ...
    if (spectral_weapon)
    {
        mprf(_("Your spectral weapon disappears as %s."),
             meld ? _("your weapon melds") : _("you unwield"));
    ...
    if (ego != SPARM_NORMAL)
    {
        ...
        case SPARM_SPIRIT_SHIELD:
            if (!unmeld && you.spirit_shield() < 2)
            {
                ...
                if (you.species == SP_DJINNI)
                    mpr(_("You feel the presence of a powerless spirit."));
                else
...
static void _equip_jewellery_effect(item_def &item, bool unmeld)
{
    ...
    case AMU_THE_GOURMAND:
        ...
        {
            mpr(_("You feel a craving for the dungeon's cuisine."));
            ident = ID_KNOWN_TYPE;
        }
        break;
