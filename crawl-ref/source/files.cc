...
static void _grab_followers()
{
    ...
    if (can_follow)
    {
        if (non_stair_using_allies > 0)
        {
            ...
            {
                mprf(_("Your summoned %s left behind."),
                     non_stair_using_allies > 1 ? pgettext("grab","allies are") : pgettext("grab","ally is"));
            }
            else
            {
                // Permanent undead are left behind but stay.
                mprf(_("Your mindless thrall%s behind."),
                     non_stair_using_allies > 1 ? pgettext("grab","s stay") : pgettext("grab"," stays"));
            }
        }
...
static bool _tagged_chunk_version_compatible(reader &inf, string* reason)
{
       ...
       )
    {
        ...
        {
            *reason = (CRAWL " " + string(Version::Short) + _(" is not compatible with "
